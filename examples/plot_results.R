...
resdir <- resdirs[2]; mdn=4; regtype = "regB"; runreg = 1; vartype = "dellog"; # numbers for testing

doplot_cpue <- function(a,vartype, mfti, regtype, runreg) {
...
# resdir = resdirs[2]; regtype = "regB2"; vartype="dellog"; runreg = 3 # testing
# resdir = resdirs[2]; regtype = "regB2"; vartype="dellog"; runreg = 3 # testing
# for (resdir in resdirs[1:4]) {
#   outdir <- paste0(resdir,"/outputs/")
#   setwd(resdir)
...
#
#         resdir = resdirs[2]; regtype = "regB2"; vartype="lognC"; runreg = 3 # testing
#         resdir = resdirs[2]; regtype = "regY"; vartype="lognC"; runreg = 5 # testing
#         outdir <- paste0(resdir,"/outputs/")
...
for (resdir in resdirs[c(7)]) {
  ...
  dir.create(outdir)
  for(regtype in c("regB")) {
    sp <- switch(regtype,regB2="bet",regY="yft",regY2="yft",regB3="bet")
    for(runreg in reglist[[regtype]]) {
      for(vartype in c("lognC")) {
        for(mdn in 1:4) {
          mdt <- c("novess_allyrs","boat_allyrs","novess_5279","vessid_79nd")[mdn]
 ...
 for (resdir in resdirs[c(7)]) {
  ...
  dir.create(outdir)
  for(regtype in c("regB")) {
    sp <- switch(regtype,regB="bet",regY="yft",regB3="bet",regY2="yft")
    for(runreg in reglist[[regtype]]) {
      for(vartype in c("lognC")) {
        for(mdn in 1:4) {
          mdt <- c("novess_allyrs","boat_allyrs","novess_5279","vessid_79nd")[mdn]
