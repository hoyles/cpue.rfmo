...
save(comp.d, file = paste0(datadir1,"jpn_raw.RData"))
nms <- c("ocean","op_yr","op_mon","op_day","lat_raw","latc","lon_raw","lonc","callsign","vesstyp","target","tonnage","mainline","branchline",
         "hbf","lbranch","lfloat","hooks","setnum","logbookid","alb","bet","yft","swo","mls","bum","blm","sas","sha","sai","spf",
         "PTS95","PTS85","PTS75","PTS65","PTS50","lat.1dgc","lon.1dgc")
rbind(names(comp.d), nms)

rawdat <- comp.d
names(rawdat) <- nms

table(comp.d$lon.1dgc)

a <- head(rawdat)
dataprep_JP_EPO(a)

# Prepare the data
pd1 <- dataprep_JP_EPO(rawdat)
...
dat_all <- pd2
dat_epo <- dat_all[dat_all$ocean == 4,]
dat_wcpo <- dat_all[dat_all$ocean == 1,]
dat <- dat_epo
