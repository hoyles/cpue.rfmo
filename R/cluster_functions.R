...
clust_PCA_run <- function(r, ddd, allsp, allabs, regtype = "regY", ncl, plotPCA = T,
                          ...
                          covarnames = c("yrqtr", "latlong", "hooks", "hbf", "vessid", "callsign", "Total", "lat", "lon", "lat5", "lon5", "moon", "op_yr", "op_mon"), saveplot = TRUE, savedir = ".") {
    ...
    pcaset <- PCA_by_set(datr, Screeplotname = paste0(fnhead, " ", "screeplot set"), allsp, saveplot, savedir = savedir)
    pcatrp <- PCA_by_trip(datr, Screeplotname = paste0(fnhead, " ", "screeplot trip"), allsp, clustid = clustid, saveplot, savedir = savedir)
    #################### Clustering
    datr <- data.frame(datr)
    cldat <- make_clusters(setdat = data.frame(datr), spp = allsp, ncl = ncl,
                           titx = paste0(flag, " ", regtype, r, " "), setclust = F,
                           tripid = clustid, fname = fnhead, regtype = regtype, saveplot = saveplot, savedir = savedir)
    plot_km_deviance_trip(ddd = datr, allsp, r = r, ti = fnhead, regtype = regtype,
                          tripid = clustid, saveplot = saveplot, savedir = savedir)
    ...
    boxplots_spCL_comp(dat = dataset, cl = "hcltrp", ti = ti, outL = F, nsp = length(allsp),
                       regtype = regtype, r = r, allsp, saveplot = saveplot, savedir = savedir)
    ...
    if (plotPCA) {
        boxplots_PCA(dat = dataset, nPCA = 4, ti = ti, dohbf = dbh, lat5 = ll5, regtype = regtype,
                     r = r, saveplot = saveplot, savedir = savedir)
        boxplots_spPCA(dat = dataset, nPCA = 4, ti = ti, nsp = 8, regtype = regtype, r = r,
                       allsp, saveplot = saveplot, savedir = savedir)
        mapPCA(ddd = dataset, nPCA = 4, ti = ti, lat5 = ll5, regtype = regtype, r = r,
               saveplot = saveplot, savedir = savedir)
        boxplots_TPCA(dat = dataset, nPCA = 4, ti = ti, dohbf = dbh, lat5 = ll5, saveplot = saveplot, savedir = savedir)
        boxplots_spTPCA(dat = dataset, nPCA = 4, ti = ti, nsp = 8, allsp, saveplot = saveplot, savedir = savedir)
        mapTPCA(ddd = dataset, nPCA = 4, ti = ti, lat5 = ll5, saveplot = saveplot, savedir = savedir)
        graphics.off()
    }
...
run_clustercode_byreg <- function(indat, reg_struc, allsp = parent.frame()$allsp,
  ...
  for(r in rgl[[reg_struc]]$allreg) {
    ...
    dataset <- clust_PCA_run(r=r,ddd=indat,allsp=allsp,allabs=allabs,regtype=reg_struc,ncl=ncl[r], plotPCA=F, clustid="tripidmon", allclust=F, flag=flag, dbh = dohbf, fnhead=fnh,covarnames=cvnames, saveplot = saveplot, savedir = basedir)
    save(dataset,file=paste0(basedir, "/", fnh,".RData"))
  }
}
...
#' @param saveplot Should be set to FALSE when working in RMarkdown.
#'
PCA_by_trip <- function(datr, Screeplotname = "screeplot set", allsp, clustid, saveplot = TRUE, savedir = ".") {
    spec_dat <- datr[, allsp]  #extract catch composition
    spec_dat$sum <- apply(spec_dat, 1, sum)
    nspec <- length(allsp)
   ...
   dev.new(noRStudioGD = TRUE)
    plotnScree(OCtesttp)
    par(col = "black")
    if (saveplot) savePlot(paste0(savedir, "/", Screeplotname, ".png"), type = "png")
...
#' @param saveplot Should be set to FALSE when working in RMarkdown.
#'
PCA_by_set <- function(datr, Screeplotname = "screeplot set", allsp, saveplot = TRUE, savedir = ".") {
  spec_dat <- datr[, allsp]  #extract catch composition
    spec_dat$sum <- apply(spec_dat, 1, sum)
    nspec <- length(allsp)
    dat2 <- datr[spec_dat$sum > 0, ]
...
#'
make_clusters <- function(setdat, spp, ncl = 5, titx = "", setclust = T, tripid = "tripid", fname = "", regtype = "regY", saveplot = TRUE, savedir = ".") {
    ...
    if (saveplot) savePlot(paste0(savedir, "/", fname, "_hclusters_trip.png"), type = "png")
    if (setclust) {
