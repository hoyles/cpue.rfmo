...
a[160:200,]
names(dat1) <- nms
table(is.na(dat1$trip_st),dat1$op_yr)
table(dat1$op_yr)
...
table(dat$vesselname,useNA="always")
...
hist(vn$bet)
by <- with(va[vn$bet > 5,], table(yft==0, op_yr))
ba <- with(va[vn$bet > 5,], table(alb==0, op_yr))
bb <- with(va[vn$bet > 5,], table(bet==0, op_yr))
windows()
yrs <- as.numeric(colnames(bb))
...
maps::map(add=T,interior=F,fill=T)
...
maps::map("worldHires",add=T, interior=F,fill=F)
...
maps::map("worldHires",add=T, interior=F,fill=T)
...
for(y in seq(1975,2015,5)) {
  ...
  maps::map("world",add=T, interior=F,fill=T) # delete data outside IO? But maybe it's just the EW code that's wrong - change to 1?
}
