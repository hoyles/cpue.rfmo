...
URdir <- paste0(projdir, "UR/")
datadir1 <- paste0(URdir, "data/catch_effort/")
jalysis_dir <- paste0(URdir, "analyses/")
URfigs <- paste0(URdir, "figures/")
...
# chk <- readLines(paste0(datadir1,"/URNLL_20170524.dat"))
# chk[10240:10242]

# Check the first 20 rows
a <- read_fwf(file=paste0(datadir1,"/URNLL_20170524.dat"),fwf_widths(wdths),col_types=cc,n_max=20);gc()
names(a) <- nms
a

# If good, load the whole file
dat5216 <- read_fwf(file=paste0(datadir1,"/URNLL_20170524.dat"),fwf_widths(wdths),col_types=cc)
...
save(dat,file="URdat.RData")
load(file="URdat.RData")
...
projdir <- "~/ICCAT/2018_CPUE/"
URdir <- paste0(projdir, "UR/")
datadir1 <- paste0(URdir, "data/")
jalysis_dir <- paste0(URdir, "analyses/")
Rdir <- paste0(projdir, "Rfiles/")
clusdir <- paste0(URdir, "clustering/")
setwd(clusdir)
load(file=paste0(jalysis_dir,"URdat.RData"))
str(dat)
...
resdir <- paste0(jalysis_dir,"std_nocl_URonly_hbf/")
dir.create(resdir)
setwd(resdir)
...
clkeepUR_B <- list("bet"=list(c(1,2,3,4),c(1,2,3,4),c(1,2,3,4)))
clkeepUS_B <- list("bet"=list(c(1,2,3,4),c(1,2,3,4),c(1,2,3,4)))
clk_B <- list(CN=clkeepCN_B,JP=clkeepJP_B,KR=clkeepKR_B,TW=clkeepTW_B,UR=clkeepUR_B,US=clkeepUS_B)
